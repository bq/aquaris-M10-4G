...
////xmwuwh @2061012 add for tp gesture gesture switch statr!
#if FTS_GESTRUE_EN
static struct proc_dir_entry *ft58xx_status_switch_proc = NULL;
unsigned char ft58xx_config[2];
unsigned int justResume_status = 0;
#define FT58XX_STATUS_SWITHC_PROC_FILE     "ft58xx_switch"
static ssize_t ft58xx_status_switch_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
    char *page = NULL;
    char *ptr = NULL;
    int len, err = -1;
    page = kmalloc(100, GFP_KERNEL);
    if (!page) {
		kfree(page);
		return -ENOMEM;
	}
    ptr = page;
    ptr += sprintf(ptr,"tp double tap is %d\n",justResume_status);
	len = ptr - page;
	if (*ppos >= len) {
		kfree(page);
		return 0;
...
static ssize_t ft58xx_status_switch_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    if (copy_from_user(&ft58xx_config[0], buffer, count))
    {
        printk("copy from user fail\n");
        return -EFAULT;
    }
    //printk("ft58xx_status_switch_write_proc, content: %d count %d\n", ft58xx_config[0],count);
    if (ft58xx_config[0] == '0')
        justResume_status = 0;
    else
        justResume_status = 1;
    return count;
...
static const struct file_operations status_switch_proc_ops = {
    .owner = THIS_MODULE,
    .read = ft58xx_status_switch_read_proc,
    .write = ft58xx_status_switch_write_proc,
};

#endif
////xmwuwh @2061012 add for tp gesture gesture switch end!
...
static int of_get_ft5x0x_platform_data(struct device *dev)
{
	...
	//tpd_rst_gpio_number = 0;//of_get_named_gpio(dev->of_node, "rst-gpio", 0);
	//tpd_int_gpio_number = 1;//of_get_named_gpio(dev->of_node, "int-gpio", 0);
	//ret = of_property_read_u32(dev->of_node, "rst-gpio", &num);
	//if (!ret)
		tpd_rst_gpio_number = 62;//num;
	//ret = of_property_read_u32(dev->of_node, "int-gpio", &num);
	//if (!ret)
		tpd_int_gpio_number = 10;//num;
...
////xmwuwh @2061012 add for tp gesture gesture switch statr!
#if FTS_GESTRUE_EN
ft58xx_status_switch_proc = proc_create(FT58XX_STATUS_SWITHC_PROC_FILE, 0666, NULL, &status_switch_proc_ops);
if (ft58xx_status_switch_proc == NULL)
{
    pr_err("create_proc_entry %s failed\n", FT58XX_STATUS_SWITHC_PROC_FILE);
}
else
{
    pr_err("create proc entry %s success", FT58XX_STATUS_SWITHC_PROC_FILE);
}
#endif
////xmwuwh @2061012 add for tp gesture gesture switch end!
...
////xmwuwh @2061012 add for tp gesture gesture switch start! 
#if FTS_GESTRUE_EN
        	if(justResume_status){
