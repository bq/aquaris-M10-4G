		...
		if (BMT_status.charger_exist == KAL_TRUE) {
			if (previous_charger_exist == KAL_FALSE) {
				...
				if ((BMT_status.charger_type == STANDARD_CHARGER)
					...
					data = batt_cust_data.ac_charger_current / 100;//AC_CHARGER_CURRENT / 100;
				else if (BMT_status.charger_type == CHARGING_HOST)
					data = batt_cust_data.charging_host_charger_current / 100;//CHARGING_HOST_CHARGER_CURRENT / 100;
				else if (BMT_status.charger_type == NONSTANDARD_CHARGER)
					data = batt_cust_data.non_std_ac_charger_current / 100;//NON_STD_AC_CHARGER_CURRENT / 100;	/* mA */
				else	/* USB */
					data = batt_cust_data.usb_charger_current / 100;//USB_CHARGER_CURRENT / 100;	/* mA */
...
//	if (upmu_is_chr_det() == KAL_TRUE) {//remove fan warning by xmtjx
...
//		if (BMT_status.charger_vol <= V_CHARGER_MIN) {
		if (batt_cust_data.v_charger_enable == 1) {
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	unsigned int hv_voltage = batt_cust_data.v_charger_max* 1000;//V_CHARGER_MAX * 1000;
	kal_bool hv_status;
