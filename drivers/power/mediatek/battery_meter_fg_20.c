...
//		if (vbat_val > VBAT_NORMAL_WAKEUP)	/* 3.6v */
		if (vbat_val > batt_meter_cust_data.vbat_normal_wakeup) /* 3.6v */
			g_spm_timer = batt_meter_cust_data.normal_wakeup_period ;//NORMAL_WAKEUP_PERIOD;	/* 54 min */
//		else if (vbat_val > VBAT_LOW_POWER_WAKEUP)	/* 3.5v */
		else if (vbat_val > batt_meter_cust_data.vbat_low_power_wakeup)	/* 3.5v */
			g_spm_timer = batt_meter_cust_data.low_power_wakeup_period ;//LOW_POWER_WAKEUP_PERIOD;	/* 5 min */
		else
			g_spm_timer = batt_meter_cust_data.close_poweroff_wakeup_period ;//CLOSE_POWEROFF_WAKEUP_PERIOD;	/* 0.5 min */
	} else
...
//	if (vbat_val > VBAT_NORMAL_WAKEUP)	/* 3.6v */
	if (vbat_val > batt_meter_cust_data.vbat_normal_wakeup)	/* 3.6v */
		g_spm_timer = batt_meter_cust_data.normal_wakeup_period;//NORMAL_WAKEUP_PERIOD;	/* 90 min */
//	else if (vbat_val > VBAT_LOW_POWER_WAKEUP)	/* 3.5v */
	else if (vbat_val > batt_meter_cust_data.vbat_low_power_wakeup)	/* 3.5v */
		g_spm_timer = batt_meter_cust_data.low_power_wakeup_period ;//LOW_POWER_WAKEUP_PERIOD;	/* 5 min */
	else
...
signed int battery_meter_get_charging_current_imm(void)
{
	...
	ICharging = (ADC_I_SENSE - ADC_BAT_SENSE + g_I_SENSE_offset) * 1000 / batt_meter_cust_data.cust_r_sense; //CUST_R_SENSE;
	return ICharging;
#endif
