...
#define VOL_1000 1000000
	...
	if (DUAL_CAMERA_MAIN_SENSOR == SensorIdx)
		...
		pinSetIdx = 2;
    printk("zhanghf %s on=%d pinSetIdx=%d %s \n",__func__,On,pinSetIdx,currSensorName);
	...
	if (On) {
		...
		if ((pinSetIdx == 1) && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV2680_MIPI_RAW))) {
			...
			if (TRUE != _hwPowerOn(VCAMIO, VOL_1800)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable IO power (VCAMIO), power id = %d\n", VCAMIO);
		...
		}else if ((pinSetIdx == 0) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_HM5040_MIPI_RAW, currSensorName))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN,
			...
			if (TRUE != _hwPowerOn(VCAMAF, VOL_2800)) {
				PK_ERR
				    ("[CAMERA SENSOR] Fail to enable digital power (VCAMAF), power id = %d\n",
				     VCAMAF);
				goto _kdCISModulePowerOn_exit_;
			}
       ...
       else if(pinSetIdx == 1 && (0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW, currSensorName)))
      {
		...
		if (pinSetIdx == 1 && currSensorName && ((0 == strcmp(currSensorName, SENSOR_DRVNAME_OV2680_MIPI_RAW)) || (0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW, currSensorName))))
		{
			/* Set Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
		...
		}else if ((pinSetIdx == 0) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_HM5040_MIPI_RAW, currSensorName) || (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV8858_MIPI_RAW)))) 
		{
			/* Set Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
				mtkcam_gpio_set(pinSetIdx, CAMRST,
            ...
            if(0 == strcmp(currSensorName, SENSOR_DRVNAME_OV8858_MIPI_RAW)){
    			if (TRUE != _hwPowerDown(VCAMAF)) {
    				PK_DBG
    				    ("[CAMERA SENSOR] Fail to OFF core power (VCAMAF),power id = %d\n",
    				     VCAMAF);
    				goto _kdCISModulePowerOn_exit_;
    			}
            }
